...
sound.all = [
	new sound.add("sounds/music2.mp3"),
	new sound.add("sounds/laser.mp3"),
	new sound.add("sounds/explosion.mp3"),
	new sound.add("sounds/explosion2.mp3"),
	new sound.add("sounds/spaceshipExplosion.mp3"),
	new sound.add("sounds/upgrade.mp3")
];
sound.loaded = false;
sound.load = function(){
	var soundsLoaded = 0;
...
game.drawLoading = function(){
	...
	ctx.font = "40pt Arial";
	ctx.textAlign = "center";
	if(frame.key >= 0 && frame.key < 25)
		ctx.fillText("LOADING", screen.width / 2, screen.height / 2);
	if(frame.key >= 25 && frame.key < 50)
		ctx.fillText("LOADING.", screen.width / 2, screen.height / 2);
	if(frame.key >= 50 && frame.key < 75)
		ctx.fillText("LOADING..", screen.width / 2, screen.height / 2);
	if(frame.key >= 75 && frame.key < 100)
		ctx.fillText("LOADING...", screen.width / 2, screen.height / 2);
	if(frame.key >= 100){
		frame.key = 0;
	}
